                           ...
                           long *errline, long *errcol)
{
  ...
  for (;;) {
    ...
    {
      unsigned char yych;
      if ((p.limit - p.cursor) < 2) { if (csv2tab_fill(input, &p, 2, &e)) break; }
      yych = *p.cursor;
      if (yych <= 0x1F) {
        if (yych <= '\t') {
...
yy3:
      ++p.cursor;
      { p.column += p.cursor - p.token; continue; }
yy4:
      ++p.cursor;
yy5:
      { goto newline; }
...
yy7:
      ++p.cursor;
      { p.column += p.cursor - p.token; goto quot; }
yy8:
      ++p.cursor;
      { p.column += p.cursor - p.token; goto comment; }
yy9:
      ++p.cursor;
      { p.column += p.cursor - p.token; ic++; continue; }
    }
...
yy22:
          yych = *++p.cursor;
          if (yych == '"') goto yy23;
          { c += p.cursor - p.token; break; }
yy23:
          ++p.cursor;
          { c += p.cursor - p.token; continue; }
        }
  ...
  cell:
    {
      ...
      for (;;) {
        ...
        {
          unsigned char yych;
          if ((p.limit - p.cursor) < 3) { if (csv2tab_fill(input, &p, 3, &e)) break; }
          yych = *p.cursor;
          if (yych <= '\f') {
            if (yych <= 0x08) {
