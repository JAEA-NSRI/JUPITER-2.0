...
/**
 * @ingroup Geometry
 * @brief Data for REC (Right Elliptic Cylinder) shape
 */
struct geom_shape_rec_data
{
  struct geom_shape_extrusion_data extr;
  double b_length; ///< Length of perpendicular vector to base
};

static const char *geom_shape_rec_descs[] = {"Base point", "Height vector",
                                             "Bottom base vector A",
                                             "Length of vector B (perp. to A)"};

static geom_variant_type geom_shape_rec_args_next(geom_args_builder *b,
                                                  ...
                                                  int *optional)
{
  ...
  geom_variant_set_string(description, geom_shape_rec_descs[l], 0);
...
static geom_error geom_shape_rec_args_check(void *p, geom_args_builder *b,
                                            ...
                                            geom_variant *errinfo)
{
      ...
      if (x <= 0.0) {
        if (errinfo) {
          geom_variant_set_string(errinfo,
                                  "Length of vector B must be positive", 0);
        } else {
  ...
  if (index == 1 || index == 2) {
    ...
    if (geom_vec3_iszero(vec)) {
      ...
      const char *ctmp;
      r =
        geom_asprintf(&tmp, "%s must not be zero", geom_shape_rec_descs[index]);
      if (r < 0) {
      ...
      if (errinfo) {
        ...
        geom_warn("(%g, %g, %g): %s", geom_vec3_x(vec), geom_vec3_y(vec),
                  geom_vec3_z(vec), ctmp);
      }
      free(tmp);
      return GEOM_ERR_RANGE;
    }
    ...
    extr = NULL;
    if (p) {
      pp = (struct geom_shape_rec_data *)p;
    } else if (b) {
...
static geom_error geom_shape_rec_set_value(void *p, geom_size_type index,
...
static geom_error geom_shape_rec_get_value(void *p, geom_size_type index,
...
static geom_size_type geom_shape_rec_n_params(void *p, geom_args_builder *b)
{
  return 4;
}

static void *geom_shape_rec_allocator(void)
{
  struct geom_shape_rec_data *p;
  p = (struct geom_shape_rec_data *)malloc(sizeof(struct geom_shape_rec_data));
  if (!p)
    return NULL;
...
static void geom_shape_rec_deallocator(void *p) { free(p); }

static void *geom_shape_rec_copy(void *p)
{
  struct geom_shape_rec_data *pp, *copy;

  pp = (struct geom_shape_rec_data *)p;
  copy = (struct geom_shape_rec_data *)geom_shape_rec_allocator();
  if (!copy)
    return NULL;
...
static int geom_shape_rec_base_plane(double x, double y, double h, void *p)
{
  struct geom_shape_rec_data *pp;
  double a, b;
  ...
  a = geom_vec3_length(pp->extr.base);
  b = pp->b_length;
  a = a * a;
  b = b * b;
...
static int geom_shape_rec_testf(void *p, double x, double y, double z)
{
  struct geom_shape_rec_data *pp;
  ...
  return geom_shape_extrusion_testf(&pp->extr, x, y, z,
                                    geom_shape_rec_base_plane, NULL, pp);
}

static geom_error geom_shape_rec_info_map(void *p, geom_info_map *list)
{
  geom_variant *v;
  geom_error e;
  struct geom_shape_rec_data *pp;
...
static geom_shape_funcs geom_shape_rec = {
  .enum_val = GEOM_SHAPE_REC,
  .shape_type = GEOM_SHPT_BODY,

  .c = {
    .allocator = geom_shape_rec_allocator,
    .deallocator = geom_shape_rec_deallocator,
    .set_value = geom_shape_rec_set_value,
    .get_value = geom_shape_rec_get_value,
    .n_params = geom_shape_rec_n_params,
    .args_next = geom_shape_rec_args_next,
    .args_check = geom_shape_rec_args_check,
    .infomap_gen = geom_shape_rec_info_map,
    .copy = geom_shape_rec_copy,
  },

  .body_testf = geom_shape_rec_testf,
...
geom_error geom_install_shape_rec(void)
{
  return geom_install_shape_func(&geom_shape_rec);
}
