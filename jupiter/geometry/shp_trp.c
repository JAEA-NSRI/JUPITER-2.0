...
/**
 * @ingroup Geometry
 * @brief Data for TRP (Truncated Right Pyramid) shape
 */
struct geom_shape_trp_data
{
...
static const char *geom_shape_trp_descs[] = {
  "Base point", "Radial vector", "Height vector",
  "Upper base factor", "Number of vertices",
};

static geom_variant_type
geom_shape_trp_args_next(geom_args_builder *b, geom_variant *description,
                         int *optional)
{
  geom_size_type l;
  ...
  geom_variant_set_string(description, geom_shape_trp_descs[l], 0);
...
static geom_error
geom_shape_trp_args_check(void *p, geom_args_builder *b, geom_size_type index,
                          const geom_variant *v, geom_variant *errinfo)
{
  ...
  if (index == 1 || index == 2) {
    ...
    extr = NULL;
    if (p) {
      pp = (struct geom_shape_trp_data *)p;
    } else if (b) {
...
static geom_error
geom_shape_trp_set_value(void *p, geom_size_type index,
                         const geom_variant *value)
{
  ...
  if (index == 3) {
    double a;
    a = geom_variant_get_double(value, &e);
    if (e != GEOM_SUCCESS) {
      return e;
    }
    pp->a = a;
...
static geom_error
geom_shape_trp_get_value(void *p, geom_size_type index,
...
geom_shape_trp_n_params(void *p, geom_args_builder *b)
{
  return 5;
}

static void *
geom_shape_trp_allocator(void)
{
  struct geom_shape_trp_data *p;
  p = (struct geom_shape_trp_data *)
    malloc(sizeof(struct geom_shape_trp_data));
  if (!p) return NULL;
...
static void
geom_shape_trp_deallocator(void *p)
{
  free(p);
}

static void *
geom_shape_trp_copy(void *p)
{
  struct geom_shape_trp_data *pp, *copy;

  pp = (struct geom_shape_trp_data *)p;
  copy = (struct geom_shape_trp_data *)geom_shape_trp_allocator();
  if (!copy) return NULL;
...
static int
geom_shape_trp_base_plane(double r, double t, double h, void *p)
{
  struct geom_shape_trp_data *pp;
  double alp, bet, nangle, rd, base;
  geom_vec3 qa, qb, vt, d, vb;
  double a, u, v;
  ...
  qa = geom_quat_rotate_vp(pp->extr.height, alp, vb);
  qb = geom_quat_rotate_vp(pp->extr.height, bet, vb);
  d  = geom_quat_rotate_vp(pp->extr.height, t,   vb);
...
static int
geom_shape_trp_testf(void *p, double x, double y, double z)
{
  struct geom_shape_trp_data *pp;
  ...
  return geom_shape_extrusion_testf(&pp->extr, x, y, z, NULL,
                                    geom_shape_trp_base_plane, pp);
}

static geom_error
geom_shape_trp_info_map(void *p, geom_info_map *list)
{
  geom_variant *v;
  geom_error e;
  struct geom_shape_trp_data *pp;
...
static
geom_shape_funcs geom_shape_trp = {
  .enum_val = GEOM_SHAPE_TRP,
  .shape_type = GEOM_SHPT_BODY,

  .c = {
    .allocator = geom_shape_trp_allocator,
    .deallocator = geom_shape_trp_deallocator,
    .set_value = geom_shape_trp_set_value,
    .get_value = geom_shape_trp_get_value,
    .n_params = geom_shape_trp_n_params,
    .args_next = geom_shape_trp_args_next,
    .args_check = geom_shape_trp_args_check,
    .infomap_gen = geom_shape_trp_info_map,
    .copy = geom_shape_trp_copy,
  },

  .body_testf = geom_shape_trp_testf,
...
geom_error geom_install_shape_trp(void)
{
  return geom_install_shape_func(&geom_shape_trp);
}
