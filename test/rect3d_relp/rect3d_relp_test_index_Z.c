...
int rect3d_index_Z_test(void)
{
  ...
  idx = rect3d_relpointer_index_Z(&cdo, 1, 0, 0, 10);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 0));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 10));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Z(&cdo, 1, 3, 0, 10);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 3));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 7));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Z(&cdo, -1, 10, 0, 25);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 15));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 10));
  test_fail_if(!test_compare_ii(idx.voffset, ==, -cdo.mxy));

  idx = rect3d_relpointer_index_Zm(&cdo, 1, 0);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 0));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 13));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zm(&cdo, 1, 11);
  ...
  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 13));
  ...
  idx = rect3d_relpointer_index_Zc(&cdo, 1, 3, 0, 0);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 0));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 6));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zc(&cdo, 1, 3, 1, 2);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 1));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 8));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zc(&cdo, 1, 5, -1, -1);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 1));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 3));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zv(&cdo, 1, 3, 0, 0);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 0));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 7));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zv(&cdo, 1, 3, 1, 2);

  test_fail_if(!test_compare_ii(idx.nbpoints, ==, 1));
  test_fail_if(!test_compare_ii(idx.nfpoints, ==, 9));
  test_fail_if(!test_compare_ii(idx.voffset, ==, cdo.mxy));

  idx = rect3d_relpointer_index_Zv(&cdo, 1, 5, -1, -1);
