...
#include "jupiter/control/postp_volume_integral.h"
#include "test-util.h"
...
int test_control_postp_volume_integral(void)
{
  ...
  vint = NULL;
  if (!test_compare_pp((vint = jcntrl_postp_volume_integral_new()), !=, NULL))
    ret = 1;
  ...
  do {
    ...
    if (!test_compare_pp((exec = jcntrl_postp_volume_integral_executive(vint)),
                         !=, NULL))
      ret = 1;
    if (ret)
      break;
    ...
    jcntrl_postp_volume_integral_set_controller(vint, world_comm);
  } while (0);
  ...
  do {
    ...
    exec = jcntrl_postp_volume_integral_executive(vint);
  ...
  do {
    ...
    switch (nproc) {
    case 1:
      if (!test_compare_dd(dval[0], ==, 6.0 * 2669.0))
        ret = 1;
      break;
    case 2:
      if (!test_compare_dd(dval[0], ==, 6.0 * (2669.0 + 3368.75)))
        ret = 1;
      break;
    case 3:
      if (!test_compare_dd(dval[0], ==, 6.0 * (2669.0 + 3368.75 + 2750.5)))
        ret = 1;
      break;
    case 4:
      if (!test_compare_dd(dval[0], ==,
                           6.0 * (2669.0 + 3368.75 + 2750.5 + 3139.0)))
        ret = 1;
      break;
    }
  } while (0);
  ...
  do {
    ...
    if (!test_compare_ii(
          jcntrl_postp_volume_integral_set_volume_varname_c(vint, "vol", 3), ==,
          1))
      ret = 1;

    if (!test_compare_ii(jcntrl_postp_volume_integral_set_cardinality_varname_c(
                           vint, "car", 3),
                         ==, 1))
      ret = 1;
  ...
  do {
    ...
    if (!test_compare_pp((d = jcntrl_cell_data_get_array_by_name(cdata, "vol",
                                                                 3)),
                         !=, NULL))
      ret = 1;
    if (ret)
    ...
    if (!test_compare_dd(dval[0], ==, 6.0 * nproc * 24))
      ret = 1;
  ...
  if (vint)
    jcntrl_postp_volume_integral_delete(vint);
